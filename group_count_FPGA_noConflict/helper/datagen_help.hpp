...
template<typename T>
bool enough_values_per_bucket(std::multimap<size_t, T> &numbers, size_t HSIZE_DATAGEN, size_t min_numbers){
...
/// @param HSIZE_DATAGEN hash table size
/// @param seed the seed for the random number generator
void next_position(size_t &pos, size_t &budget, size_t HSIZE_DATAGEN, size_t seed){
    if(budget == 0){
        budget = 1;
    }
    if(budget == 1){
        pos = (pos + 1) % HSIZE_DATAGEN;
        return;
    }
    
    size_t offset = noise(budget + pos, seed) % budget;
    budget -= offset;
    pos = (pos + 1 + offset) % HSIZE_DATAGEN;
}
...
/// @param HSIZE_DATAGEN the hash table size
...
void generate_collision(
    ...
    if(collision_lenght > HSIZE_DATAGEN || HSIZE_DATAGEN == 0){
        return;
    }

    size_t pos = start_pos % HSIZE_DATAGEN;
...
void generate_collision_soaov(
    ...
    if(collision_lenght > (HSIZE_DATAGEN * elements) || (HSIZE_DATAGEN * elements) == 0){
        return;
    }

    size_t pos = h_pos % HSIZE_DATAGEN;
    size_t data_to_generate = collision_lenght;
    ...
    while(GENERATE){
        ...
        for(it = itLow; it != itUp && GENERATE; it++){
            ...
            if(!CONTAINS){
                ...
                h_pos = (h_pos + (e_pos == 0)) % HSIZE_DATAGEN;
            }
        }
        pos = (pos + 1) % HSIZE_DATAGEN;
    }
...
void generate_cluster_soaov(
    ...
    if(cluster_lenght > (HSIZE_DATAGEN * elements) || (HSIZE_DATAGEN * elements) == 0){
        return;
    }
...
void generate_cluster(
    ...
    if(cluster_lenght > HSIZE_DATAGEN || HSIZE_DATAGEN == 0){
        return;
    }
