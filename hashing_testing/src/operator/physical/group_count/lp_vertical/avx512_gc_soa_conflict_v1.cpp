...
#include "avx512_gc_soa_conflict_v1.hpp"

#define EMPTY_SPOT 0

template <typename T>
AVX512_gc_SoA_conflict_v1<T>::AVX512_gc_SoA_conflict_v1(size_t HSIZE, size_t (*hash_function)(T, size_t))
    : Scalar_gc_SoA<T>(HSIZE, hash_function)
{}

template <typename T>
AVX512_gc_SoA_conflict_v1<T>::~AVX512_gc_SoA_conflict_v1(){
...
template <typename T>
std::string AVX512_gc_SoA_conflict_v1<T>::identify(){
    return "AVX512 Group Count SoA conflict Version 1";
}


template <typename T>
void AVX512_gc_SoA_conflict_v1<T>::create_hash_table(T* input, size_t data_size){
    size_t p = 0;
...
template <>
void AVX512_gc_SoA_conflict_v1<uint32_t>::create_hash_table(uint32_t* input, size_t data_size){
    size_t HSIZE = this->m_HSIZE;
...
template class AVX512_gc_SoA_conflict_v1<uint32_t>;
template class AVX512_gc_SoA_conflict_v1<uint64_t>;