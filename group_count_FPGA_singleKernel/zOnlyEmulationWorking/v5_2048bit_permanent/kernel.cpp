 ...
 */
void LinearProbingFPGA_variant5(queue& q, uint32_t *arr_d, uint32_t *hashVec_d, uint32_t *countVec_d, uint64_t dataSize, uint64_t HSIZE, size_t size) {
////////////////////////////////////////////////////////////////////////////////
...
//// Create a buffer on FPGA to realize buffer[] which is needed to store conflicts and hash_values within the following algorithm
	...
	// define a vector with #elementCount elements of type "Type" (normally uint32_t if nothing have been changed)
    Type buffer_data[elementCount];

	// Create a buffer, which holds #elementCount elements of of type uint32_t ("Type")
    sycl::buffer<Type> buffer_Buf(&buffer_data[0], elementCount);

	// declare an array with 64 elements to represent all possible matches found in function maskz_conflict_ret_uint64_64elements()
	// allocate the array within the shared local memory of the FPGA
	uint64_t *match_64bit = sycl::malloc_shared<uint64_t>(N, q);
...
//// starting point of the logic of the algorithm
	...
	q.submit([&](handler& h) {
		...
		// Create device accessor for the buffer.
		// The property no_init lets the runtime know that the
		// previous contents of the buffer can be discarded.
		sycl::accessor buffer(buffer_Buf, h, sycl::read_write, sycl::no_init);
...
// device_ptr<Type> buffer(buffer_d);
...
// mask_compressstoreu_epi32(buffer, negativ_no_conflicts_mask, conflicts);
	...
	#pragma unroll			// #pragma nounroll				// DO NOT UNROLL, because the steps are dependent on each other ?!
	for (int i=0; i<(regSize/sizeof(Type)); i++) {
		if (negativ_no_conflicts_mask.elements[i] == 1) {
			buffer[buffer_position] = (Type)conflicts.elements[i];
			buffer_position++;
		}
	}
...
////////////////////////////////////////////				
// instead of using an Intrinsic -> use the logic of the mask_loadu Intrinsic to
// fpvec<Type, regSize> hash_map_position = mask_loadu(no_conflicts_mask, buffer, (Type)0); // these are the hash values
	fpvec<Type, regSize> hash_map_position {};
	#pragma unroll
	for (int i=0; i<(regSize/sizeof(Type)); i++) {
		if (no_conflicts_mask.elements[i] == 1) {
			hash_map_position.elements[i] = buffer[i];
		}
		else {
			hash_map_position.elements[i] = 0;
		}
	}			
				...
				do{
					...
					*/ 
					...
					if(mask2int(foundEmpty) != 0){		//B1
						...
						fpvec<uint64_t, 512> saveConflicts = maskz_conflict_ret_uint64_64elements<Type, regSize>(foundEmpty, hash_map_position, match_64bit);
