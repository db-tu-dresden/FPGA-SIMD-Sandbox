...
fpvec<T,B> setzero() {
	...
	for(int offset=0; offset < outerUnroll; offset++) {
		...
		for (int i=0; i<innerUnroll; i++) {
			reg.elements[(offset*innerUnroll)+i] = zero;
		}
	}	
	return reg;
} 
	...
	for(int offset=0; offset < outerUnroll; offset++) {
		...
		for (int i=0; i<innerUnroll; i++) {
			if (writeMask.elements[(offset*innerUnroll)+i] == 1) {
				reg.elements[(offset*innerUnroll)+i] = data[startIndex + (offset*innerUnroll)+i];
			}
		}
	}	
	return reg;
...
fpvec<T,B> mask_cmpeq_epi32_mask(fpvec<T,B>& zeroMask, fpvec<T,B>& a, fpvec<T,B>& b) {
	...
	for(int offset=0; offset < outerUnroll; offset++) {
		#pragma unroll
		for (int i=0; i<innerUnroll; i++) {
			if (zeroMask.elements[(offset*innerUnroll)+i] == 1) {
...
fpvec<T,B> mask_add_epi32(fpvec<T,B>& src, fpvec<T,B>& writeMask, fpvec<T,B>& a, fpvec<T,B>& b) {
	...
	for(int offset=0; offset < outerUnroll; offset++) {
		...
		for (int i=0; i<innerUnroll; i++) {
			if (writeMask.elements[(offset*innerUnroll)+i] == 1) {
				reg.elements[(offset*innerUnroll)+i] = a.elements[(offset*innerUnroll)+i] + b.elements[(offset*innerUnroll)+i];
			}
			else {
...
void mask_storeu_epi32(std::array<Type, HSIZE>& result, uint32_t startIndex, fpvec<T,B>& writeMask, fpvec<T,B>& data) {
	...
	for(int offset=0; offset < outerUnroll; offset++) {
		...
		for (int i=0; i<innerUnroll; i++) {
			if (writeMask.elements[(offset*innerUnroll)+i] == 1) {
				// result[(startIndex+((offset*innerUnroll)+i))%HSIZE] = data.elements[(offset*innerUnroll)+i];
...
Type mask2int(fpvec<T,B>& mask) {
	...
	for(int offset=0; offset < outerUnroll; offset++) {
		#pragma unroll
		for (int i=0; i<innerUnroll; i++) {
			if (mask.elements[(offset*innerUnroll)+i] == 1) {
				res = 1;
				break;
			}
		}	
...
fpvec<T,B> createOverflowCorrectionMask(T oferflowUnsigned) {
	...
	for(int offset=0; offset < outerUnroll; offset++) {
		#pragma unroll
		for (int i=0; i<innerUnroll; i++) {
			if (((offset*innerUnroll)+i)<overflow) {
				reg.elements[(offset*innerUnroll)+i] = one;
			}
			else {
				reg.elements[(offset*innerUnroll)+i] = zero;
			}		
...
fpvec<T,B> createCutlowMask(T cutlowUnsigned) {
	...
	for(int offset=0; offset < outerUnroll; offset++) {
		#pragma unroll
		for (int i=0; i<innerUnroll; i++) {
			if ((offset*innerUnroll)+i<cutlow_const) {
				reg.elements[(offset*innerUnroll)+i] = zero;
			}
			else {
				reg.elements[(offset*innerUnroll)+i] = one;
